...
class _CommunityScreenState extends State<CommunityScreen> {
  ...
  Future<void> fetchCommunityPosts() async {
    const String endpoint = "http://192.168.137.129:8000/community/posts";
  ...
  Future<void> _startVoice() async {
    ...
    stt.listen(
      onResult: (result) {
        searchCtrl.text = result.recognizedWords;
        setState(() {});
      },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            const GreenTopBar(titleEn: "Community", titleHi: "समुदाय"),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceAround,
              children: [
                _category(Icons.agriculture, t("Crops", "फसलें")),
                _category(Icons.bug_report, t("Disease Detector", "रोग पहचान")),
                _category(Icons.yard, t("Fertilization", "उर्वरीकरण")),
                _category(Icons.attach_money, t("Market Price", "बाज़ार मूल्य")),
              ],
            ),
            const SizedBox(height: 8),
            ...
            Padding(
              ...
              child: Align(
                alignment: Alignment.centerLeft,
                child: Text(
                  "${t("Community", "समुदाय")}: ${t("Crops", "फसलें")}",
                  style: const TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                  ),
  ...
  Widget _buildPostsList() {
    ...
    if (posts.isEmpty) {
      return Center(child: Text(t("No posts found", "कोई पोस्ट नहीं मिली")));
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // 🔍 IMPROVED SMART SEARCH — works Hindi + English
        final search = searchCtrl.text.trim().toLowerCase();
        final combinedText = "$name $title $content ${tags.join(' ')}"
            .toLowerCase();

        if (search.isNotEmpty) {
          final words = search.split(" ");
          bool matches = words.every((w) => combinedText.contains(w));
          if (!matches) return const SizedBox();
        }

        // Time formatting
        String timeLabel = createdAt;
        if (createdAt.contains("T")) {
          timeLabel = createdAt.replaceFirst("T", " ").substring(0, 16);
        }

        return _postCard(
          name.isEmpty ? t("Unknown", "अज्ञात") : name,
          timeLabel,
          title,
          content,
          tags,
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            content.isNotEmpty
                ? content
                : t("No description available", "कोई विवरण उपलब्ध नहीं"),
          ...
          Text(
            t(
              "record breaking tech news on groundbreaking partnership, stocks and in skyrockets.",
              "नई तकनीक व साझेदारी से जुड़ी रिकॉर्ड ब्रेकिंग खबरें, स्टॉक्स और तेज़ उछाल।",
            ),
